simulation_prob_death <- function(position_matrix, payoff_matrix, num_generations,
  ...
  going_to_die <- function(position_matrix, fitness_matrix, w = 1) {
    intermediate_matrix <- matrix(NA, nrow = size, ncol = size)
    for (i in 1:size) {
      for (j in 1:size) {
        prob_death <- 1 / (1 + (exp(1)^(w*fitness_matrix[i,j])))
        a <- runif(1, 0, 1)
        if (a < prob_death) {
  ...
  for (generation in 1:num_generations) {
    ...
    intermediate_matrix <- going_to_die(position_matrix, fitness_matrix)
