simulation_prob_death <- function(position_matrix, payoff_matrix, num_generations,
                                  strategy, divide, k) {
  ...
  get_fitness_matrix <- function(position_matrix, divide, strategy) {
  ...
  going_to_die <- function(position_matrix, fitness_matrix, k) {
    intermediate_matrix <- matrix(NA, nrow = size, ncol = size)
    for (i in 1:size) {
      for (j in 1:size) {
        d <- 1 / (1 + exp(k * fitness_matrix[i,j]))
        prob_death <- 2 * d  # normalizando para variar entre 1 e 0
        a <- runif(1, 0, 1)
        if (a < prob_death) {
  ...
  for (generation in 1:num_generations) {
    ...
    fitness_matrix <- get_fitness_matrix(position_matrix, divide, strategy)
    
    # step 2: select agents to reproduce and die
    intermediate_matrix <- going_to_die(position_matrix, fitness_matrix, k)
