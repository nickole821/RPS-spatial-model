...
df_freq <- read.csv("./output/data/stochastic/diferent-neighborhood/adaptative-neighborhood/p025.csv",
                     header = T)
...
dir.create("./output/data/stochastic/regular-neighborhood/prob_reproduce/mean/")


write.csv(df_media,
          "./output/data/stochastic/diferent-neighborhood/adaptative-neighborhood/mean/mean_p025.csv")
...
freq_p025 <- ggplot(df_freq, aes(x = Generation, y = freq, color = as.factor(strategy),
  ...
  theme_minimal() +
  labs(x = "Gerações", y = "Frequência", title = "adap, divide = FALSE, p = 0.25") +
  scale_color_manual(values = c("O" = "tomato2", "Y" = "gold2", "B" = "blue"),
                     name = "Estratégia", labels = c("Azul", "Laranja", "Amarelo")) +
  theme(text = element_text(size = 24)) +
  scale_y_continuous(lim = c(0,1))

ggsave(filename = "./output/data/stochastic/diferent-neighborhood/adaptative-neighborhood/mean/p025.pdf",
