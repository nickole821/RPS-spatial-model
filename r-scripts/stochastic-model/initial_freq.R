...
dir.create("./output/data/stochastic/simetric_payoff-matrix/initial_freq/O01-Y01-B08")
...
for (i in 1:n_sim) {
  ...
  result <- simulation(position_matrix, payoff_matrix, num_generations = 100,
                       prob_reproduce, strategy = FALSE)
  
  pasta_resultados <- file.path("./output/data/stochastic/initial_freq/02-04-04/",
                                paste0("sim_", i))
